...
if option == "-station":
    station = sys.argv[2]
    dfs = []
    df = iofilehandler.txtfile2dataframe(config['general']['outputfolder']+station+'_original_sampling_flags.out', None, [0,1], "\"%Y-%m-%d%H:%M:%S\"", 2, 3)
    ...
    df = iofilehandler.txtfile2dataframe(config['general']['outputfolder']+station+'_5min_interpolated.out', None, [0,1], "\"%Y-%m-%d%H:%M:%S\"", 2, None)
    dfs.append([df[['data']],'5-min interpolation','#2980b9',312,1.2,'o','solid', 0, 0.05, 0.5])
    df = iofilehandler.txtfile2dataframe(config['general']['outputfolder']+station+'_hourly_slev.out', None, [0,1], "\"%Y-%m-%d%H:%M:%S\"", 2, None)
    dfs.append([df[['data']],'Hourly SLEV','#FA8072',312,3.5,'o','solid', 0, 0.05, 0.5])
    df = iofilehandler.txtfile2dataframe(config['general']['outputfolder']+station+'_hourly_surge.out', None, [0,1], "\"%Y-%m-%d%H:%M:%S\"", 2, None)
    ...
    for file in files:
        try:
            df = iofilehandler.txtfile2dataframe(file, None, [0,1], "\"%Y-%m-%d%H:%M:%S\"", 2, 3)
            ...
            df = iofilehandler.txtfile2dataframe(file, None, [0,1], "\"%Y-%m-%d%H:%M:%S\"", 2, None)
